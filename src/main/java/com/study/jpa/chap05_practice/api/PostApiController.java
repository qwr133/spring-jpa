 ...
 */
    ...
    public ResponseEntity<?> create(@Validated @RequestBody PostCreateDTO dto,
                                    BindingResult result){ //검증에러 정보를 가직 객체
        ...
        if(result.hasErrors()){ //입력값 검증에 걸림 ex, 제목을 안쓰거나~
        ...
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.internalServerError().body("미안, 서버 터짐 원임 : " + e.getMessage());
        }
